...
let connectionLines = {};
let radiusOverlays  = [];
let hasExplored = false; // true after the first pill click — suppresses the prompt on location switch
...
// Fill the custom location dropdown and handle switching
function buildDropdown() {
  ...
  // Set initial label to first location name
  if (LOCATIONS.length) label.textContent = LOCATIONS[0].name;
...
function renderKmlPills(loc) {
  ...
  const wrap      = container.closest('[data-pills-wrap]');

  // If the user has already explored, keep the prompt hidden across location switches
  if (wrap && hasExplored) wrap.setAttribute('data-explored', '');
  ...
  loc.kmlLayers.forEach(layer => {
    ...
    pill.addEventListener('click', () => {
      ...
      if (!hasExplored) {
        hasExplored = true;
        wrap?.setAttribute('data-explored', '');
      }
...
function extractFeatures(parent) {
  ...
  Array.from(parent.querySelectorAll('Placemark')).forEach(pm => {
    ...
    const desc = rawDesc
      .split(/[\n\r]+/)
      .map(line => line.replace(/^address:\s*/i, ''))
      .filter(line => {
        const l = line.trim();
